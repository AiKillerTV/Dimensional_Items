...
public class IObsidianLamp extends Block{

	private final boolean isOn;
	
	public IObsidianLamp(boolean isOn) {
	...
	public void onBlockAdded(World world, int x, int y, int z){
		if(!world.isRemote){
			if(!this.isOn && world.isBlockIndirectlyGettingPowered(x, y, z)){
				...
				world.setBlock(x, y, z, DIBlocks.blockIOLampOn, 0, 2);
			}
		}
	}
	...
	public void updateTick(World world, int x, int y, int z, Random random){
		...
		if(!world.isRemote && !this.isOn && world.isBlockIndirectlyGettingPowered(x, y, z)){
			world.setBlock(x, y, z, DIBlocks.blockIOLampOff, 0, 2);
		}
	}
	
	public Item getItemDropped(int meta, Random random, int Fortune){
		return Item.getItemFromBlock(DIBlocks.blockIOLampOn);
	}
	
	protected ItemStack createStackedBlock(int i){
		return new ItemStack(DIBlocks.blockIOLampOn);
	}
